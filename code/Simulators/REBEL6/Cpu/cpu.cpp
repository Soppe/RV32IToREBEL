...
namespace
{
   ...
   const std::int32_t ProgramEndRAValue = -std::pow(3, 16); // Just some random negative number where only a single trit is set, minimizing the impact it has on tritflips
...
namespace Simulators
{
   ...
   {
      CPU::CPU()
          : m_registers(RegisterCount), m_PC(0), m_numberOfRanInstructions(0)
      {
      }
      ...
      void CPU::executeRegister(const std::string &name, bool isBinary, const std::string &rd, const std::string &rs1, const std::string &rs2)
      {
         ...
         {
            if (name == "add")
               InstructionExecutor::executeAdd(rdVal, rs1i, rs2i);
            else if (name == "sub")
               InstructionExecutor::executeSub(rdVal, rs1i, rs2i);
            else if (name == "sll")
               InstructionExecutor::executeSll(rdVal, rs1i, rs2i);
            else if (name == "srl")
               InstructionExecutor::executeSrl(rdVal, rs1i, rs2i);
            else if (name == "sra")
               InstructionExecutor::executeSra(rdVal, rs1i, rs2i);
            else if (name == "sltu")
               InstructionExecutor::executeSltu(rdVal, rs1i, rs2i);
            else if (name == "or")
               InstructionExecutor::executeOr(rdVal, rs1i, rs2i);
            else if (name == "xor")
               InstructionExecutor::executeXor(rdVal, rs1i, rs2i);
            else if (name == "and")
               InstructionExecutor::executeAnd(rdVal, rs1i, rs2i);
            else
            {
         ...
         else
         {
            ...
            // else if(name == "sr.t")  InstructionExecutor::executeSr_t(rdVal, rs1i, rs2i);
               ...
               InstructionExecutor::executeSlt_t(rdVal, rs1i, rs2i);
      ...
      void CPU::executeImmediate(const std::string &name, bool isBinary, const std::string &rd, const std::string &rs1, const std::string &imm)
      {
         ...
         {
            if (name == "addi")
               InstructionExecutor::executeAddi(rdVal, rs1i, immi);
            else if (name == "slli")
               InstructionExecutor::executeSlli(rdVal, rs1i, immi);
            else if (name == "srli")
               InstructionExecutor::executeSrli(rdVal, rs1i, immi);
            else if (name == "srai")
               InstructionExecutor::executeSrai(rdVal, rs1i, immi);
            else if (name == "sltiu")
               InstructionExecutor::executeSltiu(rdVal, rs1i, immi);
            else if (name == "ori")
               InstructionExecutor::executeOri(rdVal, rs1i, immi);
            else if (name == "xori")
               InstructionExecutor::executeXori(rdVal, rs1i, immi);
            else if (name == "andi")
               InstructionExecutor::executeAndi(rdVal, rs1i, immi);
            else
            {
         ...
         {
            if (name == "addi.t")
               InstructionExecutor::executeAddi_t(rdVal, rs1i, immi);
            ...
            // else if(name == "sri.t")  InstructionExecutor::executeSri_t(rdVal, rs1i, immi);
               ...
               InstructionExecutor::executeSlti_t(rdVal, rs1i, immi);
      ...
      void CPU::executeLoadImmediate(const std::string &name, const std::string &rd, const std::string &imm)
      {
         ...
         if (name == "li.t")
            InstructionExecutor::executeLi_t(rdVal, immi);
         else if (name == "aipc.t")
            InstructionExecutor::executeAipc_t(rdVal, immi, m_PC);
         else
         {
            std::cerr << __PRETTY_FUNC__ << ": Unsupported ternary immediate instruction " << name << std::endl;
            abort();
      ...
      void CPU::executeBranch(const std::string &name, bool isBinary, std::uint8_t instructionSize, const std::string &rs1, const std::string &rs2, const std::string &offset)
      {
         ...
         {
            if (name == "bgeu")
               InstructionExecutor::executeBgeu(rs1i, rs2i, offi16, pcVal, instructionSize);
            else if (name == "bltu")
               InstructionExecutor::executeBltu(rs1i, rs2i, offi16, pcVal, instructionSize);
            else
            {
               std::cerr << __PRETTY_FUNC__ << ": Unsupported binary immediate instruction " << name << std::endl;
               abort();
         ...
         {
            if (name == "beq.t")
               InstructionExecutor::executeBeq_t(rs1i, rs2i, offi16, pcVal, instructionSize);
            else if (name == "bne.t")
               InstructionExecutor::executeBne_t(rs1i, rs2i, offi16, pcVal, instructionSize);
            else if (name == "blt.t")
               InstructionExecutor::executeBlt_t(rs1i, rs2i, offi16, pcVal, instructionSize);
            else if (name == "bge.t")
               InstructionExecutor::executeBge_t(rs1i, rs2i, offi16, pcVal, instructionSize);
            else
            {
      ...
      void CPU::executeJump(const std::string &name, std::uint8_t instructionSize, const std::string &rd, const std::string &offset)
      {
         ...
         if (name == "jal.t")
            InstructionExecutor::executeJal_t(rdVal, offseti, pcVal, instructionSize);
         else
         {
      ...
      void CPU::executeJumpRegister(const std::string &name, std::uint8_t instructionSize, const std::string &rd, const std::string &target)
      {
         ...
         if (name == "jalr.t")
            InstructionExecutor::executeJalr_t(rdVal, offset, rs1, pcVal, instructionSize);
         else
         {
      ...
      void CPU::executeLoad(const std::string &name, bool isBinary, const std::string &rd, const std::string &address, ExecutableProgram &program)
      {
         ...
         {
            if (name == "lw")
               InstructionExecutor::executeLw(rdVal, offset, rs1, program);
            else if (name == "lh")
               InstructionExecutor::executeLh(rdVal, offset, rs1, program);
            else if (name == "lb")
               InstructionExecutor::executeLb(rdVal, offset, rs1, program);
            else if (name == "lhu")
               InstructionExecutor::executeLhu(rdVal, offset, rs1, program);
            else if (name == "lbu")
               InstructionExecutor::executeLbu(rdVal, offset, rs1, program);
            else
            {
      ...
      void CPU::executeStore(const std::string &name, bool isBinary, const std::string &rs, const std::string &address, ExecutableProgram &program)
      {
         ...
         {
            if (name == "sw")
               InstructionExecutor::executeSw(rs2, offset, rs1, program);
            else if (name == "sh")
               InstructionExecutor::executeSh(rs2, offset, rs1, program);
            else if (name == "sb")
               InstructionExecutor::executeSb(rs2, offset, rs1, program);
            else
            {
